...
LinearLayer::LinearLayer(int in_dim, int out_dim, bool has_bias) : has_bias(has_bias), data(in_dim, out_dim) {
  if (has_bias)
...
std::vector<Value> LinearLayer::get_parameters() {
  std::vector<Value> params(this->data.in_dim * this->data.out_dim + this->bias.out_dim);
